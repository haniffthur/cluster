            ...
            <form action="{{ route('cards.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="card_number" class="form-control @error('card_number') is-invalid @enderror"
                        value="{{ old('card_number') }}" placeholder="Tempel kartu pada reader atau ketik manual..."
                        required autofocus>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="kategori" id="kategori" class="form-control">
                                <option value="penghuni" {{ (isset($card) && $card->kategori == 'penghuni') ? 'selected' : '' }}>Penghuni Cluster</option>
                                <option value="tamu" {{ (isset($card) && $card->kategori == 'tamu') ? 'selected' : '' }}>Tamu
                                    / Umum</option>
                                <option value="security" {{ (isset($card) && $card->kategori == 'security') ? 'selected' : '' }}>Security / Staff</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6">
    ...
    <script>
        $(document).ready(function () {
            ...
            function toggleResident() {
                // Hanya tampilkan dropdown penghuni jika kategori = 'penghuni'
                if ($('#kategori').val() === 'penghuni') {
                    $('#resident-wrapper').show();
                } else {
                    // Tamu & Security sembunyikan dropdown
